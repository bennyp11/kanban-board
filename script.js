...
function getSavedColumns() {
  if (localStorage.getItem(!backlogListArray.length === 0)) {
...
//add to column list, reset ttextbox
function addtoColumn(column){
 const itemText = addItems[column].textContent;
 const selectedArray = listArrays[column];
 selectedArray.push(itemText);
...
//when the item enters the column area, this functino dragEnter will take the column as the parameter
function dragEnter(column) {
  listColumns[column].classList.add('over');
  currentColumn = column;
}
...
//column allowing for the items to be dropped into themselves
function allowDrop(e) {
  e.preventDefault();
}

//dropping items into the column
function drop(e) {
  e.preventDefault();
  const parent = listColumns[currentColumn];
